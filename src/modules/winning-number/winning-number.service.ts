...
export class WinningNumberService {
  ...
  async fetch(lastestEpisode: number): Promise<void> {
    ...
    const lt365s: Lt365[] = response1.data.data.list;
    ...
    const lt365: Lt365 = response2.data.data.list.pop()!;
    lt365Map.set(lt365.ltEpsd, lt365);
  ...
  /**
   * Find all winning numbers from WINNING_NUMBER table.
   */
  ...
  /**
   * Find one winning number from WINNING_NUMBER table by episode.
   */
